...
export default function AdminCameraForm({ camera, onSave, onCancel }) {
  const [data, setData] = useState({ lat: camera?.location.lat ?? 13.7563, lng: camera?.location.lng ?? 100.5018,
    ...
    connection_type: camera?.connection_type || 'wifi_local', camera_ip: camera?.camera_ip || '', rtsp_path: camera?.rtsp_path || '/stream1',
  ...
  return <form className="cctv-form" onSubmit={async event => {
    ...
    <div className="cctv-fields">
      ...
      <label>การเชื่อมต่อ<select value={data.connection_type} onChange={e => change('connection_type', e.target.value)}><option value="lan_ip">LAN / IP</option><option value="wifi_local">WiFi ภายใน</option><option value="onvif">ONVIF</option></select></label>
